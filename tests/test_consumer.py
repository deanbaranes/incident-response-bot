...
from workers.incident_consumer import send_to_dlq, consume, processed_incidents
...
@pytest.mark.asyncio
@patch("workers.incident_consumer.USE_KAFKA_QUEUE", True)
...
async def test_consume_success(
    mock_start_http_server, mock_process_incident, mock_consumer_cls, mock_producer_cls
):
    ...
    assert "test-1" in processed_incidents
...
async def test_consume_exception_routes_to_dlq(
    ...
    assert "test-error" not in processed_incidents
